# Screening Attention — ベンチマークレポート

**実装**: `multiscreen` v0.1.0
**論文**: "Screening Is Enough" (arXiv:[2604.01178](https://arxiv.org/abs/2604.01178)) — Ken M. Nakanishi
**実行環境**: NVIDIA GeForce RTX 4060 Ti (16GB VRAM) / PyTorch 2.5.1+cu121 / Python 3.11

---

## 評価概要

| ベンチマーク | ツール | 目的 |
|---|---|---|
| A: 計算効率 | `torch.utils.benchmark`（PyTorch公式） | レイテンシ・スループット・メモリ |
| B: 言語モデル品質 | WikiText-2 perplexity（標準LMベンチマーク） | 学習品質の比較 |

---

## A: 計算効率ベンチマーク

`d_model=512, num_heads=8, batch=4, causal=True`

### A-1. レイテンシ (ms) — `torch.utils.benchmark.blocked_autorange`

| seq_len | ScreeningAttn | nn.MHA | F.SDPA |
|--------:|-------------:|-------:|-------:|
| 128 | 1.693 | 0.538 | **0.357** |
| 256 | 1.772 | 0.575 | **0.350** |
| 512 | 2.747 | 0.924 | **0.694** |
| 1,024 | 11.200 | 2.654 | **1.754** |
| 2,048 | 43.060 | 8.394 | **4.849** |
| 4,096 | 1,956.2 | 29.623 | **14.840** |

### A-2. スループット (tokens/sec)

| seq_len | ScreeningAttn | nn.MHA | F.SDPA |
|--------:|--------------:|-------:|-------:|
| 128 | 302,475 | 950,789 | **1,435,256** |
| 512 | 745,494 | 2,216,418 | **2,951,230** |
| 1,024 | 365,719 | 1,543,431 | **2,335,499** |
| 2,048 | 190,248 | 975,903 | **1,689,287** |
| 4,096 | 8,376 | 553,075 | **1,104,043** |

### A-3. ピークGPUメモリ (MB)

| seq_len | ScreeningAttn | nn.MHA | F.SDPA |
|--------:|--------------:|-------:|-------:|
| 128 | 23.8 | 20.1 | **19.0** |
| 512 | 138.6 | 43.4 | **37.9** |
| 1,024 | 482.5 | 77.6 | **63.0** |
| 2,048 | 1,824.7 | 152.0 | **113.4** |
| 4,096 | 7,126.3 | 331.5 | **214.0** |

### A-4. スパース性分析

初期化時（s_r=0 → r=2, s_v=0 → w=2）のアルファ行列スパース率：

| seq_len | r=2 (初期値) | r=4 | r=8 |
|--------:|------------:|----:|----:|
| 128〜2048 | **100.0%** | **100.0%** | **100.0%** |

初期化時に全キーがスクリーンされる。理由：
1. **Trim-and-Square の閾値**: r=2 のとき `sim > 0.5` を要求。ランダム単位ベクトル（64次元）のコサイン類似度は0付近に集中するため全てゼロ
2. **ウィンドウサイズ w=2**: 初期値ではコサインマスクが直前1トークンのみを許容

### A まとめ

**ナイーブな PyTorch 実装は nn.MultiheadAttention より 3〜66 倍遅く、メモリも 5〜21 倍多く使用する。**

これは論文の主張（最大3.2×高速化）と逆だが、矛盾ではない。論文の高速化は以下を前提としている：

> *"The screening mechanism sets many alpha values to exactly zero, enabling sparse value aggregation via a custom CUDA kernel that skips screened-out keys."*

本実装はアルファ行列を密に計算した上でゼロ乗算を行うため、スパース性の恩恵を受けられない。スパース率100%という結果は、カスタムカーネルがあれば**理論的な高速化余地は非常に大きい**ことを示している。

---

## B: WikiText-2 言語モデル品質ベンチマーク

### 実験設定

| 項目 | 値 |
|------|-----|
| データセット | WikiText-2 (Salesforce/wikitext, raw-v1) |
| トークナイザー | キャラクターレベル (vocab_size=1,156) |
| コンテキスト長 | 256 |
| バッチサイズ | 32 |
| モデル規模 | d_model=256, heads=4, layers=4 |
| 学習ステップ | 5,000 |
| 学習率 | 3e-4 (AdamW + CosineAnnealing) |
| 評価指標 | Perplexity (PPL) ↓ 低いほど良い |

### 学習曲線

**TransformerLM (softmax)**

| step | train_ppl | valid_ppl |
|-----:|----------:|----------:|
| 500 | 10.88 | 10.12 |
| 1,000 | 7.73 | 7.07 |
| 2,000 | 6.09 | 5.54 |
| 3,000 | 5.27 | 5.12 |
| 4,000 | 5.07 | 4.93 |
| 5,000 | 4.91 | **4.90** |

**MultiscreenLM (screening)**

| step | train_ppl | valid_ppl |
|-----:|----------:|----------:|
| 500 | 9.31 | 8.59 |
| 1,000 | 7.70 | 7.42 |
| 2,000 | 6.34 | 6.18 |
| 3,000 | 5.69 | 5.64 |
| 4,000 | 5.66 | 5.43 |
| 5,000 | 5.69 | **5.42** |

### 最終結果

| モデル | パラメータ数 | test PPL | 学習時間 |
|--------|------------:|----------:|--------:|
| TransformerLM (softmax) | 3,521,024 | **4.71** | 197s |
| MultiscreenLM (screening) | 3,509,536 | 5.28 | 342s |

- MultiscreenLM のパラメータ数は TransformerLM より **0.3% 少ない**（ほぼ同規模）
- test PPL は MultiscreenLM が **12% 悪化**、学習時間は **74% 長い**

### B まとめ

今回の条件ではスクリーニング機構が品質面でも不利だった。ただし解釈には注意が必要：

1. **スクリーニング機構の初期化問題**: 初期化時スパース率100%のため、序盤の学習でAttentionがほぼ無効化された状態から始まる。FFNのみで学習が進む期間が長く、収束が遅い（valid_pplの推移に顕著）

2. **論文との条件差異**:
   - 論文はワードレベルトークン + より大規模なモデル（数億パラメータ）
   - 論文は十分なウォームアップ（初期化時のスパース性問題に対応する学習率スケジュール）
   - 本実験は5,000ステップのみ（論文は収束まで学習）

3. **パラメータ効率の検証不可**: 論文の「40%少ないパラメータで同等のloss」という主張は、今回の実験規模・ステップ数では再現できない

---

## 総合考察

### 論文の主張 vs 本実装の結果

| 主張 | 論文 | 本実装 |
|------|------|--------|
| 計算速度 | 最大3.2×高速化 | 3〜66倍**低速** |
| パラメータ効率 | ~40%削減で同等loss | 同規模で12%悪化 |
| メモリ | 削減を示唆 | 5〜21倍増加 |

### 乖離の根本原因

```
論文の高速化
  └── スパースなアルファ行列を活かすカスタムCUDAカーネル
        ├── ゼロのキーを完全スキップ
        └── Flash Attention的なメモリ効率化

本実装
  └── ナイーブなdense行列演算
        ├── 全キーを計算してから0をかける（無駄）
        └── O(T²) メモリ（softmaskの追加確保も含む）
```

スパース率100%という結果は**逆説的に好材料**でもある。カスタムカーネルが実装されれば、スクリーンされた全てのキーの計算をスキップできるため、理論上の高速化余地は極めて大きい。

### 実用化に必要な次のステップ

1. **カスタムCUDAカーネル**: スパースなアルファ行列の計算・集約を効率化（Triton/CUDA）
2. **初期化戦略の改善**: s_r を負の初期値にして r を小さく始める（序盤からAttentionを有効化）
3. **学習率スケジュール**: スクリーニング機構の固有の学習ダイナミクスに合わせたウォームアップ

---

## 再現方法

```bash
pip install -e ".[dev]"

# A: 計算効率
python benchmarks/bench_efficiency.py
python benchmarks/bench_sparsity.py

# B: WikiText-2品質
python benchmarks/bench_wikitext2.py
```

結果JSON: `benchmarks/results_efficiency.json`, `results_sparsity.json`, `results_wikitext2.json`
